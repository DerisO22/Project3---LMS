...
app.post("/courses", (req, res) => {
  const { CoursePrefix, CourseNumber, RoomNumber, Building, StartTime } =
    req.body;
  db.run(
    ...
    (err, row) => {
      ...
      if (row) {
        return res.status(400).json({
          error: `Student is already enrolled in ${row.CoursePrefix}-${row.CourseNumber}`,
        });
      }
  ...
  db.get(
    ...
    (err, row) => {
      ...
      if (!row)
        return res
          .status(400)
          .json({ err: "Student isn't enrolled in this course" });

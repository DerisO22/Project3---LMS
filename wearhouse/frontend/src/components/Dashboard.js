...
export default function Dashboard({ isAdmin }) {
  ...
  const addOrUpdateData = async (data, type) => {
    ...
    if (type === "student_courses" || type === "student_courses_card") {
      isEdit = editStudentCourses;
    }
    if (type === "grades") {
      isEdit = editStudentGrades;
    }
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `Failed to ${isEdit ? "update" : "add"} ${type.replace("_", " ")}`
        );
      }
      setNotification({
        show: true,
  ...
  const handleDeleteTableData = async (id, table_name, studentID) => {
    if (table_name === "student_courses") {
      await fetch(`http://localhost:3000/${table_name}/${studentID}/${id}`, {
        method: "DELETE",
      });
    } else {
      await fetch(`http://localhost:3000/${table_name}/${id}`, {
        method: "DELETE",
      });
    }
    fetchData();
